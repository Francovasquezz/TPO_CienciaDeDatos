...
import { usePlayerDetail } from '../api/hooks';
import { PlayerHeader } from '../components/PlayerHeader';
import { PlayerStatsTable } from '../components/PlayerStatsTable';

// IMPORTACIONES DE UI CON RUTAS RELATIVAS (DEBE SER: ../components/ui/card)
import { Card, CardHeader, CardTitle, CardContent } from '../components/ui/card';

// La importación de Badge fue eliminada porque no se usa en esta página.

// Componente para los Partidos Recientes (Requires Card)
...
// Componente para el Aporte del Modelo (Feature Attribution) - Requires Card
...
export const PlayerDetailPage: React.FC = () => {
  ...
  const { data: player, isLoading, isError } = usePlayerDetail(playerId);

  if (isLoading) {
    return <div className="p-4">Cargando detalles del jugador...</div>;
  }
  if (isError) {
    return <div className="p-4 text-red-600">Error al cargar el detalle del jugador.</div>;
  }
  if (!player) {
  ...
  return (
    <div className="space-y-6">
      <Link to="/players" className="text-sm text-blue-600 hover:underline">&larr; Volver a la lista</Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          {/* 3. Aportantes del Modelo (Si es estimado) */}
        ...
        <div className="lg:col-span-2">
          {/* 4. Tabla de Rendimiento (GK / Campo) */}
